...
class CompanyAddr(Addr):
    def __init__(self, name, phone, email, address, birth):
        super().__init__(name, phone, email, address, birth, "회사")
        company = input("회사명을 입력해주세요: ")
        depart = input("부서명을 입력해주세요: ")
        position = input("직급을 입력해주세요: ")
        self.extra_info = f"회사명: {company}\n부서: {depart}\n직급: {position}"
...
class CustomerAddr(Addr):
    def __init__(self, name, phone, email, address, birth):
        super().__init__(name, phone, email, address, birth, "거래처")
        company = input("거래처명을 입력해주세요: ")
        depart = input("품목을 입력해주세요: ")
        position = input("직급을 입력해주세요: ")
        self.extra_info = f"거래처명: {company}\n품목: {depart}\n직급: {position}"
...
class SmartPhone:
    ...
    def input_addr_data(self):
        ...
        if group == "회사":
            addr = CompanyAddr(name, phone, email, address, birth)
        elif group == "거래처":
            addr = CustomerAddr(name, phone, email, address, birth)
        return addr
